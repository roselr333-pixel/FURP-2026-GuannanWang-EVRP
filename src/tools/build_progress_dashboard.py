...
<div class="wrap">
  ...
  <section>
    ...
    <figure>
      ...
      （距离 −32%、时长 −34%）。加权和标量化贪心，前沿为"粗"前沿，非完整 Pareto。</figcaption>
    </figure>
    ...
    <figure>
      ...
      卸载率 11.3%→5.2%→2.6% 下降，N=100 时 V2 平均 88.8/100 顾客超时（TW 可行性崩溃）。有效协同区间收窄到 N≤30。</figcaption>
    </figure>
  </section>
  ...
  <section>
    ...
    <ul>
      <li><b>主线：</b>协作增益主源是"多顾客能力"而非"会合次数"——受控消融(+8.6~12.1pp；标准算例上 +7.2~8.9pp) 与敏感性 K 扫描(20.5%→37.1%) 两个独立实验结论一致。</li>
      ...
      <li><b>局限：</b>加权和贪心非完整 Pareto 求解；规模受限于合成算例（N=100 时 TW 可行性崩溃，有效区间 N≤30）；未复现 MILP 下界。</li>
    </ul>
  </section>

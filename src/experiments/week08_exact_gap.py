...
and reports the gap of each heuristic to the optimum. It also runs the project's
ORIGINAL V2 / LNS (the shared evaluator `week07_fstsp_repro`) and records whether
those sortie sets are physically valid, because the shared evaluator serialises
sorties with a `drone_avail` chain without checking that the drone is back on the
truck — so a nested sortie set is given a value that no feasible plan realises.

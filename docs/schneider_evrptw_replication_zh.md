...
18 个对照实例（全部完全服务）平均距离差距 **+48.9%**，其中：

| 集合 | 距离差距（均值） | 车辆数（我方 vs BKS） |
|---|---|---|
| C5 小实例（5 顾客） | **约 +25%**（区间 −4.4% ~ +45.8%） | 略多 0–2 车 |
| `_21` 大实例（100 顾客） | **+53% ~ +233%** | 多 5–11 车 |

差距的真实来源（我都如实写，不夸大）：

1. **车辆数未最小化（主因）**。Schneider 的首要目标是最小化车辆数，且其车辆可
   **多程**（一次出库跑多条路线）；构造式贪心无法全局安排多程，故车辆数明显偏多，
   直接拉高总距离。例如 `c201_21`：我方 15 车 vs BKS 4 车；`rc201_21`：15 车 vs 4 车；
   `r201_21`：13 车 vs 3 车。大实例的距离差距绝大部分来自这里，而非路线几何。

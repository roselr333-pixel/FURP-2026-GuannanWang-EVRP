...
电池不够则经最近充电站满充后继续）。目标按 Schneider 的层级：先最小化车辆数、再最小化距离。
...
- **求解时间**：每个实例 < 0.02 s（最慢 rc201_21 约 0.01 s），在 20 核机器上测得；
  构造式启发式算力开销可忽略，差距来自算法质量而非算力。

## 5. 与 BKS 的差距（归因）

18 个对照实例（全部完全服务）平均距离差距 **+50.7%**（绝对值均值；有符号均值 +50.1%）。
求解器自 2026-09-10 起是确定的（顾客集按 ID 排序迭代），这些数字重跑可精确复现。拆分如下：

| 集合 | 距离差距（均值） | 车辆数（我方 vs BKS） |
|---|---|---|
| C5 小实例（5 顾客） | **约 +21%**（区间 −4.4% ~ +45.8%） | 略多 0–2 车 |
| `_21` 大实例（100 顾客） | **+54% ~ +239%** | 多 5–12 车 |

差距的真实来源：

1. **车辆数未最小化（主因）**。Schneider 的首要目标是最小化车辆数，且其车辆可
   **多程**（一次出库跑多条路线）；构造式贪心无法全局安排多程，故车辆数明显偏多，
   直接拉高总距离。例如 `c201_21`：我方 16 车 vs BKS 4 车；`rc201_21`：15 车 vs 4 车；
   `r201_21`：14 车 vs 3 车。大实例的距离差距绝大部分来自这里，而非路线几何。
2. **启发式简单**。最近可行插入，路线几何非最优，顾客分配有碎片。
3. **距离度量**。我用原始欧氏距离；论文可能截断到 1 位小数，差异 < 0.1%，可忽略。
4. **关于 `c103C5` 我方距离略低于 BKS**：这不是实例不同（已核对一致），而是 Schneider
   层级目标下 BKS 选了 **更少车辆（m=1）而接受略高距离（176.05）**，我的 2 车解距离
   175.4 略低但车辆更多，按论文目标我方更差——符合预期。

## 6. 局限与后续任务

- RC1 紧时间窗下 1 顾客未服务；要补上需要更重的插入搜索或 ALNS。
- **多程已建模，但最小化车辆数（把顾客全局分配给多程车队）未实现**——这是缩小与 BKS 差距的
  关键杠杆；我把它留作后续扩展（同侪 Ziqi / Frank 用 ALNS / 元启发式正是为此）。

...
Files produced: `src/results/baseline_ga_vrptw_results.csv`, `src/results/baseline_ga_vrptw_comparison.csv` (GA vs OR-Tools vs BKS), `src/results/baseline_ga_vrptw_output.txt` and the console transcript `src/results/baseline_ga_multi_seed.log` (both written by the script; the older single-seed transcript `src/results/baseline_ga_run.log` is reproduced with `GA_SEEDS=20260717 python src/experiments/baseline_ga_vrptw.py`, which writes the `*_single_seed.*` artifacts instead of the multi-seed ones).

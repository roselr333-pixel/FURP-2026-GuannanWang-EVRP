...
17. ~~**No coherent "why it works" chain**~~ ✅ **Delivered**: `docs/week06_depth_analysis_zh.md` chains the ablation (multi-customer ability as main source, +6.6~17.7pp), the sensitivity K scan (25.3%→56.5%), the applicability regime, two-axis MO dominance, three-baseline positioning, failure cases (N=50 ~4.5M sync rejections), and limitations into one argument; accompanied by `docs/experiment_evidence_index_zh.md` and `experiment_evidence_index_en.md` collecting every citable number with provenance.

...
- **主评估器已物理化，W7/W8 已重跑**：`week07_fstsp_repro.fstsp_simulate` 现在拒绝嵌套/交叉架次（返回 inf），W7/W8 及依赖它的 W6 灵敏度/多目标已全部重跑；数值整体下调但结论方向不变（详见 `docs/evaluator_physical_fix_note_zh.md`）。**本轮未覆盖**：`week06_ground_air_evrp_tw.py` 自带的并行评估器（比旧 `fstsp_simulate` 更宽松）与 `week06_largeN` 未改动，因此 W6 的 "V2 vs V1 −424（p=3.7×10⁻⁸）" 与规模衰减 27.6%→12.1%→2.3% 仍是旧口径，属待统一整改项。

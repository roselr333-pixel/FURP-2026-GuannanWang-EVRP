...
无人机越多收益越高、并压平规模衰减（均匀随机的 R101 收益最高、聚类的 C101 最低）。**调度**：把"架次→无人机"分配当显式调度，greedy（最早可用）vs local（局部搜索）vs optimal（分支定界，架次 ≤ 14）+ 下界证书（每架次独占一架）——local 相对 greedy 平均只 **+0.001%**；**31/64 配置上 greedy == 下界，即 naive 规则可证明最优**；精确最优对照（43 个配置）naive 平均只高 **0.196%（max 5.2%）**。架次很多时下界偏松，理论收益上界均值 3.23%（max 40.1%），这部分未证明。出处 `week08_multidrone_std.py` + `drone_scheduling.py` + `fstsp_instances.py`；图 `figures/multidrone_std.png`；说明 `docs/week08_multidrone_std_note_zh.md`。
...
**V3：电动卡车 + 无人机 + 充电站 + 时间窗**（说明 `docs/v3_ev_collab_note_zh.md`）：把 week06 的电池 + 充电 + 时间窗叠回协同模型。路线只列顾客、充电绕行由评估器动态插入，因此 LNS 的 destroy/repair 原样复用。每规模 10 种子：

| 规模 | V1 纯电卡车 | V3+LNS | 提升 | V1 时间窗违例 → V3 | V1 充电次数 → V3 |
|---|---:|---:|---:|---:|---:|
| N=8 | 485.8 | 67.8 | **86.0%** | 2.4 → **0.0** | 0.9 → **0.0** |
| N=20 | 1283.6 | 551.7 | **56.7%** | 13.5 → **1.0** | 2.9 → **1.3** |

**电动 + 时间窗约束放大了无人机价值**：收益（56.7%~87.8%）远高于同一批方法在 FSTSP 设定下的 23%~45%，因为卸载缩短了卡车路线，顺带省掉大部分充电绕行并消掉大部分时间窗违例。出处 `v3_ev_collab.py` → `v3_ev_collab_summary.csv`。
...
- **V2 的电池/充电层已由 V3 叠回**：V3（电动卡车 + 无人机 + 充电站 + 时间窗）已实现并与 V1 对比（见 §8）；仍缺的是把**多无人机**接到 V3 上。
...
- **无人机调度：约一半配置可证明最优，大规模仍未证明**：用"每架次独占一架"的下界做证书，31/64 配置 greedy 触到下界（可证明最优），精确最优对照（架次 ≤ 14）naive 平均只高 0.196%。但架次很多时下界偏松，理论收益上界均值 3.23%、最大 40.08%——**大规模多无人机的调度最优性尚未严格证明**。
- **W7/W8 的时间窗/电量已由 V3 补上**：V3 把 week06 的电池、充电站与时间窗叠回卡车-无人机模型（见 §8），该设定下协同收益达 56.7%~87.8% 并消掉大部分时间窗违例与充电绕行。V3 目前仍是单架无人机，且时间窗在搜索里是罚项而非硬约束（报告里如实给出违例数）。

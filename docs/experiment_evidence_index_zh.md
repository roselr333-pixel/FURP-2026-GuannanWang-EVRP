...
- **贪心无局部搜索**：V2 加 2-opt 后处理增益仅 0–1.76%（size 16 最高 1.76%），说明增益来自协同结构而非路线微调。
- **每架次限 2–3 顾客**：简化也是边界；更大规模多顾客服务需更重搜索。
- **合成算例规模 ≤ 50**：未接入真实大规模基准（如 ESOGU），结论外推需谨慎。
- **未复现 MILP 下界**：基线对比用 BKS（文献最优）而非自证下界，绝对质量靠文献锚定。
- **多目标为粗前沿**：加权和贪心，非完整 Pareto 求解器（见 §3）。

...
## 7. Schneider (2014) E-VRPTW 复现（构造式贪心 + 局部搜索 vs BKS）
...
（允许等待）+ 充电站满充下做可行性检查；随后由 `schneider_improve.py` 在同一层级目标下精化
（最省绕行充电站修复、整趟搬移/合并、顾客 relocate/swap、2-opt）。图：`figures/schneider_routes.png`（4 个代表实例的
...
聚合（18 个有 BKS 的实例）：
- 平均距离差距 **+21.3%**（绝对值均值，局部搜索之后），构造阶段单独是 **+52.7%**；其中 3 个实例
  （c208C5、r105C5、rc208C5）与 BKS 完全一致。
- 平均车辆数：我方 **3.28** vs BKS **2.06**（构造阶段是 5.44）。剩余差距集中在 100 顾客 _21 集
  （均值 +43.6%，最差 c201_21 +97.2%，6 车 vs 4 车）：剩下的是**车队装填**，不是路线几何。
- 加局部搜索时顺带发现的建模 bug：新开车辆的首趟从未检查仓库关闭时间（18 个实例里有 3 个超时
  回仓）。现在每一趟都强制检查，构造基线因此从 +50.7% 移到 +52.7%。

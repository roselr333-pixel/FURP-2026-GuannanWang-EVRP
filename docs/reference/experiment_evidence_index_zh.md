...
运行已热启动（把启发式最好的计划同时作为目标截断与 `AddHint` 传入），n=8 每实例 120 s、n=10 起 60 s。n=8 一行与冷启动运行完全一致（同样五个最优值），说明提示没有扰动模型。
...
- **精确最优只证到 n=8，再往上证明停在对偶界上**：用 CP-SAT 求物理 FSTSP 模型——n=8 已证明最优（贪心 +31.2%、LNS +16.5%）；n=10–16 热启动后能给出比我的启发式低 8.8–17.5% 的可行计划，但认证对偶界塌到 0，所以那些 gap 仍是真实 gap 的下界。更大规模的精确最优/下界仍是开放项，也是模型可改进的方向。基线的绝对质量仍以 BKS（文献最优）锚定。

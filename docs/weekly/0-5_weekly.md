...
- *Added 2026-07-14:* upgraded the truck-drone model to **v2** (`src/experiments/week05_truck_drone_v2.py`) — the drone is carried by the truck and launched/recovered at **any** node (FSTSP-style) with range + rendezvous constraints. Under a physical single-drone schedule (one sortie at a time) the makespan is **318.9, 17.9% below truck-only**; on this six-customer instance it is still slower than v1's model, in which the drone flies its own depot loop independently (270.6), because a carried drone has to be recovered by the truck. The carried-drone model pays off at larger sizes (W6-W8).
...
- Produced:
  ...
  - a **short report** (Setup / Results / Discussion / Conclusion + failure analysis): `docs/weekly/week03_report.md`
...
**Links (optional):** `src/experiments/week03_experiment.py`, `src/results/week03_summary_table.csv`, `src/results/week03_experiment_log.txt`, `docs/weekly/week03_report.md`, `src/results/week03_route_n20_vrptw_improved.png`
...
- *The Week 1 baseline smoke test (required deliverable) was actually completed on 2026-07-07 during the catch-up sprint* — see `docs/weekly/week01_checkpoint.md`.
